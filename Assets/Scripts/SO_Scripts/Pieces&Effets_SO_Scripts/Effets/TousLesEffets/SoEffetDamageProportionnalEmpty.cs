...
[CreateAssetMenu(fileName = "SoEffetDamageProportionnalEmptyEffet", menuName = "Effet/DamageProportionnalEmpty")]

public class SoEffetDamageProportionnalEmpty : SoEffet
{
    public override IEnumerator Effet(Context context, OutputPort port, List<int> amount, int tour)
    {
        port.piecePlayed.PiecePlayedUp();
        port.thisBoardPiece.soPieces.TempEffectValues[0] += context.NbrCaseLibre;
        port.DoDamageToEnnemi(amount[0] + port.thisBoardPiece.soPieces.TempEffectValues[0]);
